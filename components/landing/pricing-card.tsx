...
export default function PricingCard({ plan, onSelect }: PricingCardProps) {
  return (
    <article
      className={`relative h-full overflow-hidden rounded-2xl border px-6 py-4 transition-[transform,border-color,box-shadow,background-color] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transform-none md:rounded-3xl md:p-5 lg:p-[1.15rem] ${
        plan.featured
      ...
      <div className="flex h-full flex-col space-y-3.5 lg:space-y-3">
        ...
        <div className="mt-auto pt-1.5 lg:pt-2">
          <button
            ...
            className={`w-full rounded-xl border py-2.5 text-sm font-semibold transition-[transform,background-color,border-color,box-shadow] duration-150 ease-out active:scale-[0.96] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300/40 motion-reduce:transform-none ${
              plan.featured
